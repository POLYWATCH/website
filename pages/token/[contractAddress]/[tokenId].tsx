...
export default function TokenPage({ nft, contractMetadata }: Props) {
  ...
  // Funzione chiamata quando il pulsante "Buy at asking price" viene premuto
  const handleBuyButtonClick = () => {
    // Verifica se ci sono listing validi
    if (allValidListings.length === 0) {
      console.error("No valid listings available");
      return;
    }

    // Seleziona casualmente un NFT dalla lista
    const randomIndex = Math.floor(Math.random() * allValidListings.length);
    const selectedNFT = allValidListings[randomIndex];

    // Esegui qui la logica per l'acquisto o altre azioni con l'NFT selezionato
          ...
          <div className={styles.listingContainer}>
            ...
            {loadingContract || loadingDirect || loadingAuction ? (
              ...
              <>
               ...
               <Web3Button
        contractAddress={MARKETPLACE_ADDRESS}
        action={async () => await handleBuyButtonClick()}
        className={styles.btn}
        onSuccess={() => {
          toast(`Purchase success!`, {
            icon: "✅",
